...
export async function settingsConversation(
  ...
  await ctx.reply(formatSettings(user), { reply_markup: buildSettingsKeyboard() })
  ...
  while (true) {
    ...
    if (action === 'set_timezone') {
      ...
      await ctx.reply(`✅ Часовой пояс изменён: ${user.timezone}`, { reply_markup: buildSettingsKeyboard() })
      ...
      await ctx.reply(`✅ Утренний план: ${user.morning_time}`, { reply_markup: buildSettingsKeyboard() })
      ...
      if (!hasGoogleConfig) {
        await ctx.reply('Google Calendar временно недоступен.', { reply_markup: buildSettingsKeyboard() })
        continue
      }
      ...
      try {
        ...
        await ctx.reply('Google Calendar подключён ✅', { reply_markup: buildSettingsKeyboard() })
      } catch (err) {
        logger.error('[conversation/settings] getAuthUrl failed', {
          userId: user.id,
          error: err instanceof Error ? err.message : String(err),
        })
        await ctx.reply('Не удалось получить ссылку. Попробуй позже.', { reply_markup: buildSettingsKeyboard() })
      }
    }
  }
